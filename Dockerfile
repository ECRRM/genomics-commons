...
LABEL description="ECRRM Bioinformatics Docker Image"
...
##########################################################################################
##########################################################################################
# Adding additional repositories for software installation
...
# Setting up a temporary directory for Docker setup
##########################################################################################
##########################################################################################
...
RUN cd $SETUPDIR/ && \
    ...
    echo "echo 'Welcome to ECRRM Genomics Toolbox (v1.0)'" >> $SETUPDIR/init.sh && \
    echo "echo '****************************************'" >> $SETUPDIR/init.sh && \
    echo "echo 'ECRRM Genomics Toolbox is a docker image for genomics tools'" >> $SETUPDIR/init.sh && \
    echo "echo " >> $SETUPDIR/init.sh && \
    echo "echo 'For a list of installed tools, please visit: '" >> $SETUPDIR/init.sh && \
    echo "echo 'https://github.com/ECRRM/genomics-toolbox/blob/master/Tools.md'" >> $SETUPDIR/init.sh && \
    echo "echo " >> $SETUPDIR/init.sh && \
    echo "echo 'If you would like to request adding certain tools or report a problem,'" >> $SETUPDIR/init.sh && \
    echo "echo 'please submit an issue https://github.com/ECRRM/genomics-toolbox/issues'" >> $SETUPDIR/init.sh && \
    echo "echo " >> $SETUPDIR/init.sh && \
    echo "echo 'If you use ECRRM Genomics Toolbox in your work, please cite: '" >> $SETUPDIR/init.sh && \
    echo "echo '10.5281/zenodo.8103969'"  >> $SETUPDIR/init.sh && \
    echo "echo 'Have fun!'" >> $SETUPDIR/init.sh && \
    echo "echo ''" >> $SETUPDIR/init.sh && \
    echo "echo ''" >> $SETUPDIR/init.sh && \
    echo "/usr/bin/bash" >> $SETUPDIR/init.sh && \
    echo "" >> $SETUPDIR/init.sh && \
    mv $SETUPDIR/init.sh /etc/genomics-toolbox.sh && \
    chmod a+x /etc/genomics-toolbox.sh && \
