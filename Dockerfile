...
LABEL version="1.0"
LABEL description="Genomics Commons Docker Image containing a collection of genomics tools"
# Add any other metadata labels as needed
# Configure non-interactive mode for package installations
RUN echo 'debconf debconf/frontend select Noninteractive' | debconf-set-selections

# Update system, add necessary repositories
RUN apt-get update --fix-missing &&     apt-get -y upgrade &&     apt-get -y install apt-utils dialog software-properties-common &&     add-apt-repository universe &&     add-apt-repository multiverse &&     add-apt-repository restricted
